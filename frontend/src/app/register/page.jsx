...
export default function Register() {
  ...
  const [formData, setFormData] = useState({
    name: '', email: '', phoneNumber: '', password: '', otpMethod: 'email',
  });
  ...
  return (
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .otp-btn {
          ...
          border: 1px solid #c8c4bc; background: transparent;
          color: #888; cursor: pointer; transition: all 0.15s;
        }
        .otp-btn.active { border-color: #0d0d0d; color: #0d0d0d; background: #0d0d0d1a; }
        .otp-btn:not(.active):hover { border-color: #aaa; color: #555; }
        .two-col { display: grid; grid-template-columns: 1fr 1fr; gap: 2rem; }
        @media (max-width: 500px) { .two-col { grid-template-columns: 1fr; gap: 1.5rem; } }
      `}</style>
      ...
      <div style={{ minHeight: '100vh', width: '100%', background: '#f0ede6', color: '#0d0d0d', display: 'grid', gridTemplateColumns: '1fr 1fr' }}>
        ...
        >
          ...
          <div>
            ...
            <h1 className="fd" style={{ fontSize: 'clamp(3rem, 5vw, 5rem)', fontWeight: 700, lineHeight: 0.92, letterSpacing: '-0.01em', color: '#0d0d0d', marginBottom: '1.8rem' }}>
              Create your<br />
              <em style={{ fontWeight: 400, fontStyle: 'italic', color: '#d4400a' }}>secure</em><br />
              account.
            </h1>
            <p className="fm" style={{ fontSize: 11, color: '#888', lineHeight: 1.8, maxWidth: '30ch' }}>
              Choose your verification method — OTP via email or SMS through Twilio.
            </p>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.9rem' }}>
            {[
              { n: '01', label: 'Fill in your details' },
              { n: '02', label: 'Choose OTP method' },
              { n: '03', label: 'Verify your identity' },
            ].map((step) => (
              <div key={step.n} style={{ display: 'flex', alignItems: 'center', gap: '0.8rem' }}>
                <span className="fm" style={{ fontSize: 9, color: '#bbb' }}>{step.n}</span>
                <div style={{ flex: 1, height: 1, background: '#d6d3cc' }} />
                <span className="fm" style={{ fontSize: 10, color: '#888', letterSpacing: '0.1em' }}>{step.label}</span>
              </div>
            ))}
          </div>
        </motion.div>
        ...
        {/* Right — form panel */}
        <div style={{ padding: '3rem 6%', display: 'flex', flexDirection: 'column', justifyContent: 'center', overflowY: 'auto' }}>
          <motion.div {...fade(0.1)} style={{ maxWidth: 440, width: '100%' }}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              {/* Name + Phone row */}
              <motion.div {...fade(0.16)} className="two-col" style={{ marginBottom: '2rem' }}>
                <div>
                  ...
                  <input type="text" name="name" placeholder="Jane Smith" className="field" required onChange={handleChange} />
                </div>
                <div>
                  <label className="fm" style={{ fontSize: 10, letterSpacing: '0.22em', textTransform: 'uppercase', color: '#888', display: 'block', marginBottom: '0.6rem' }}>
                    Phone
                  </label>
                  <input type="tel" name="phoneNumber" placeholder="+1 555 000 0000" className="field" required onChange={handleChange} />
                </div>
              </motion.div>

              {/* Email */}
              <motion.div {...fade(0.22)} style={{ marginBottom: '2rem' }}>
                <label className="fm" style={{ fontSize: 10, letterSpacing: '0.22em', textTransform: 'uppercase', color: '#888', display: 'block', marginBottom: '0.6rem' }}>
                  Email address
                </label>
                <input type="email" name="email" placeholder="name@example.com" className="field" required onChange={handleChange} />
              </motion.div>

              {/* Password */}
              <motion.div {...fade(0.28)} style={{ marginBottom: '2.5rem' }}>
                <label className="fm" style={{ fontSize: 10, letterSpacing: '0.22em', textTransform: 'uppercase', color: '#888', display: 'block', marginBottom: '0.6rem' }}>
                  Password
                </label>
                <input type="password" name="password" placeholder="••••••••" className="field" required onChange={handleChange} />
              </motion.div>

              {/* OTP method */}
              <motion.div {...fade(0.33)}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '0.9rem' }}>
                  <span className="fm" style={{ fontSize: 10, letterSpacing: '0.22em', textTransform: 'uppercase', color: '#888' }}>
                    Verification via
                  </span>
                  <div style={{ flex: 1, height: 1, background: '#d6d3cc' }} />
                </div>
                <div style={{ display: 'flex', gap: '0.6rem' }}>
                  {['email', 'sms'].map((method) => (
                    <button
                      key={method}
                      type="button"
                      className={`otp-btn${formData.otpMethod === method ? ' active' : ''}`}
                      onClick={() => setFormData({ ...formData, otpMethod: method })}
                    >
            ...
            {/* Footer link */}
            <motion.div {...fade(0.44)} style={{ marginTop: '2rem', paddingTop: '2rem', borderTop: '1px solid #d6d3cc' }}>
              <p className="fm" style={{ fontSize: 11, color: '#888' }}>
                Already have an account?{' '}
                <Link href="/login" style={{ color: '#0d0d0d', fontWeight: 500, textDecoration: 'none', borderBottom: '1px solid #0d0d0d' }}>
                  Sign in
                </Link>
              </p>
            </motion.div>
